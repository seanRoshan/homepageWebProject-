...
<body>
	...
	<div class="container" id="bodyContainer">
		...
		$sql_re = "SELECT * FROM `researchExperiences` WHERE 1 ORDER BY `startDateYear` desc , `endDateMonth` desc";
		...
		while ($row_re = mysqli_fetch_array($result_re)) {
				...
				$advisor = $row_re['labAdvisor'];
				$advisorURL = $row_re['labAdvisorURL'];


				$jobStartDateMonth = num_to_month($row_re['startDateMonth']);
				$jobStartDateYear = $row_re['startDateYear'];
				$jobEndDateMonth = num_to_month($row_re['endDateMonth']);
				$jobEndDateYear = $row_re['endDateYear'];

				$jobDescription = $row_re['labDescription'];

				$target_file_pdf = $row_re['pdfDir'];
				$target_file_ppt = $row_re['pptDir'];

				$jobID = $row_re['researchID'];
				...
				echo '<i class="fa fa-flask fa-lg goldText" aria-hidden="true">&nbsp</i>'.$researchTitle;
				echo '</span></div>';
				echo '<div class="row">';
				...
				echo '</div></div>';
				echo '<div class="col">';
				echo '<span class="teaching_location_date_text text-muted">'.'<a href="'.$labURL.'" target="_blank" class="bioLink text-muted">'.$labTitle.'</a>'.'</span>';
				echo '<br>';
				echo '<span class="teaching_location_date_text text-muted">'.$labLocation.'</span>';
				...
				echo '<br>';
				echo '<br>';
				echo '<span class="teaching_location_date_text text-muted">'.'<a href="'.$advisorURL.'" target="_blank" class="bioLink text-muted"> Supervisor:&nbsp'.$advisor.'</a>'.'</span>';
				...
				echo '<a href="'.$target_file_ppt.'" class="fa fa-file-powerpoint-o fa-2x bioLink skillTag" target="_blank">&nbsp</a>';
